-- ============================================================
-- Migration 042: Filtrar partidos de OTRAS competiciones en la forma
--
-- Bug: refresh_team_statistics() unía matches por team_id sin exigir
-- que el partido perteneciera a la MISMA competición del equipo. Como
-- las selecciones reutilizan el mismo team_id entre el Mundial
-- (a1b2c3d4) y los amistosos pre-mundial (f1f2f3f4), los resultados
-- de amistosos se colaban en "forma reciente", xG, xGA y rachas del
-- torneo — afectando a los 48 equipos.
--
-- Ejemplo real: la derrota amistosa 2-1 ante Argentina (marzo, otra
-- competición) aparecía como una "P" en la forma oficial de Colombia
-- para el Mundial, y su xG/xGA promediaba partidos que no son del
-- torneo. Viola la Regla #2 (separar contextos) del plan aprobado.
-- ============================================================
...
BEGIN
  WITH team_matches AS (
    ...
    JOIN matches m ON m.status = 'finished' AND m.home_score IS NOT NULL
      ...
      AND m.competition_id = t.competition_id  -- ← el fix: solo partidos del MISMO torneo
...
-- Verificación:
--   SELECT refresh_team_statistics();  -- reprocesa con el filtro correcto
--   SELECT array_to_string(form,'') FROM team_statistics ts
--   JOIN teams t ON t.id=ts.team_id WHERE t.code='COL';
--   -- esperado: solo W/D del Mundial, sin la L amistosa vs ARG
