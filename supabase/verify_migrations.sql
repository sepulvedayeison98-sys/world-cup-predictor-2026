...
-- 045: acotado a fútbol — desde la 048 la NBA también es type='league'.
UNION ALL SELECT '045 cinco grandes ligas', (SELECT count(*)=5 FROM competitions WHERE type='league' AND season='2024-25' AND sport_id=(SELECT id FROM sports WHERE slug='football')) AND (SELECT count(*)=34 FROM (SELECT DISTINCT round FROM matches WHERE competition_id='78000000-0000-4000-8000-000000000078' AND round IS NOT NULL) s)
